# AI Agent Guidelines for goods-price-comparison-api
...
### Using Podman Desktop (Recommended for GUI)

**For AI agents preferring visual feedback:**

```bash
# Install Podman Desktop
brew install podman-desktop

# Initialize (macOS/Windows only)
podman machine init
...
1. **Lint** - Spectral validation
2. **Build** - Docker multi-stage build
3. **Test** - Run all tests in container
4. **Quality** - Checkstyle + SpotBugs
5. **Publish** - Push to GitHub Packages

**AI agents:** Test locally with Podman before pushing:
```bash
podman build -t test-build . && echo "✅ Container build passed"
```
...
### 3. OpenAPI Specification Linting

**Spectral Rules (MUST pass before PR):**
...
- Tags must be defined in global tags section
